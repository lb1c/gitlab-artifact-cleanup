...
class GitlabArtifactCleanup(object):
    def __init__(self, dry_run=False, min_age=None, keep=None):
        self.dry_run = dry_run
        self.min_age = timedelta() if min_age is None else min_age
        self.keep = 0 if keep is None else keep
    ...
    def cleanup_project(self, proj):
        ...
        for job in proj.jobs.list(as_list=True):
            try:
...
def parse_args():
    ...
    ap.add_argument('--keep', action="store", help="keep the last n artifacts", type=int)
    args = ap.parse_args()
